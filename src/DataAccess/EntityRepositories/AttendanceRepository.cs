...
internal class AttendanceRepository : ISudentAttendanceRepository
{
    ...
    public async Task DeleteEntityAsync(int attendanceId)
    {
        ...
        await _context.SaveChangesAsync();
        _logger.LogInformation($"Deleted attendance id = {attendanceId} from database.");
    }
    ...
    public async Task<IReadOnlyCollection<IAttendance>> GetAllEntitiesAsync()
    {
        ...
        // не выкидывать эксепшены, а отдавать пустую коллекцию
        _ = attendances.Length == 0 ? throw new MissingMemberException($"Cannot find any {nameof(Attendance)} members.") : 0;

        _logger.LogInformation($"Get all members {nameof(Attendance)} from database.");
        return _mapper.Map<IReadOnlyCollection<Attendance>>(attendances);
    }

    public async Task<IReadOnlyCollection<IAttendance>> GetAttandanceByFirstLastName(string firstName, string lastName)
    {
        var attendances = await _context.Attendances
            .Include(i => i.Student)
            .Where(j =>
                EF.Functions.ILike(j.Student.FirstName, firstName)
                && EF.Functions.ILike(j.Student.LastName, lastName))
            .Include(i => i.Lecture)
            .ToArrayAsync();
    ...
    private async Task<(LectureDb Lecture, StudentDb Student)> CheckValidAttandanceAsync(IAttendance attendance)
    {
        ...
        var lecture = await _context.Lectures?.FirstOrDefaultAsync(i => i.Topic == attendance.LectureTopic)!
                      ?? throw new NullReferenceException();

        var student = await _context.Students?.FirstOrDefaultAsync(i =>
                          i.FirstName == attendance.StudentFirstName && i.LastName == attendance.StudentLastName)!
                      ?? throw new NullReferenceException();
